...
function PlaidButton({ onSuccess, onExit, label = "Connect a Bank" }) {
  ...
  const { open, ready } = usePlaidLink({
    token: linkToken,
    onSuccess: (publicToken, metadata) => onSuccess(publicToken, metadata?.institution?.name),
    onExit,
  });
...
export default function App() {
  ...
  function applyRules(txns, currentRules) {
    ...
    return txns.map(t => {
      ...
      for (const rule of currentRules) {
        ...
        else match = merchant.includes(pattern);
  ...
  function saveManualTxn() {
    ...
    setTransactions(p=>[{
      id:"m"+Date.now(), date:txnForm.date, merchant:txnForm.merchant.trim(), name:"",
