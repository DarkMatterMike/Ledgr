...
export default function App() {
  ...
  const [rulePrompt,   setRulePrompt]   = useState(null); // { txnId, merchant, categoryId }
  ...
  useEffect(() => { if (!loading) scheduleSave({ rules });        }, [rules]);
  ...
  function applyRules(txns, currentRules) {
    if (!currentRules || currentRules.length === 0) return txns;
    return txns.map(t => {
      // Don't overwrite manually set categories unless rule is forced
      ...
      for (const rule of currentRules) {
        ...
        if (!pattern) continue;
        let match = false;
        if (rule.matchType === "exact")    match = merchant === pattern;
        else if (rule.matchType === "starts") match = merchant.startsWith(pattern);
        else                               match = merchant.includes(pattern); // contains
        if (match) {
          return {
            ...t,
  ...
  function saveRule(rule) {
    ...
    showToast("Rule saved — will apply on next sync");
  }
  ...
  async function doSync(itemId) {
    ...
    try {
      ...
      setTransactions(prev => {
        ...
        // Merge added — apply rules to new transactions
        ...
        const newTxns = applyRules(rawNew, rules);
        return [...newTxns, ...next];
      });
...
function capitalise(s) { return s ? s.charAt(0).toUpperCase()+s.slice(1) : ""; }
  ...
  /* Budgets */
  const [drillCat, setDrillCat] = useState(null); // category being drilled into

  const catTxns = useMemo(() =>
    drillCat ? monthTxns.filter(t => t.categoryId === drillCat.id && t.amount < 0)
               .sort((a,b) => b.date.localeCompare(a.date))
    : [],
  [drillCat, monthTxns]);
    ...
    <div>
      ...
      {categories.length===0
        ...
        : <div className="ledgr-budget-grid" style={S.grid2}>
            {categories.map(cat=>{
              ...
              return (
                <div key={cat.id} style={{...S.card,borderLeft:`3px solid ${cat.color}`,cursor:"pointer",transition:"border-color 0.15s, box-shadow 0.15s"}}
                  onClick={()=>setDrillCat(cat)}
                  onMouseEnter={e=>e.currentTarget.style.boxShadow=`0 0 0 1px ${cat.color}44`}
      ...
      {drillCat && (
        <div style={S.overlay} onClick={e=>e.target===e.currentTarget&&setDrillCat(null)}>
          <div style={{...S.modal, width:620, maxHeight:"80vh", display:"flex", flexDirection:"column"}}>
                    ...
                    {catTxns.map(t=>(
                      <tr key={t.id}>
                        ...
                        <td style={{...S.td,padding:"10px 10px"}}>
                          <select
                            style={{...S.select,width:"100%",fontSize:11,padding:"5px 8px",
                              borderColor: "var(--border2)",
                              background:"var(--surface)"}}
                            value={t.categoryId||""}
                            onChange={e=>{
                              updateTxnCat(t.id, e.target.value);
                              // Update drillCat ref so the list refreshes
                              if (e.target.value !== drillCat.id) {
                                showToast("Transaction moved");
                              }
                            }}>
                            {categories.map(c=>(
                              <option key={c.id} value={c.id}
                                style={{fontWeight:c.id===drillCat.id?"700":"400"}}>
                                {c.id===drillCat.id?"✓ ":""}{c.name}
                              </option>
                            ))}
                            <option value="">— Uncategorized —</option>
                          </select>
            ...
            {/* Footer */}
            <div style={{marginTop:16,paddingTop:16,borderTop:"1px solid var(--border)",display:"flex",justifyContent:"flex-end",flexShrink:0}}>
    ...
    <div>
      ...
      <div style={{marginTop:24,padding:"16px 20px",background:"var(--surface)",border:"1px solid var(--border)",borderRadius:"var(--radius-lg)"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8,fontSize:13,color:"var(--t2)"}}>
          <div><span style={{fontFamily:"var(--font-mono)",color:"var(--cyan)",marginRight:8}}>contains</span>Merchant name includes the text anywhere. "whole" matches "Whole Foods".</div>
  ...
  const RuleModal = (
    <Modal title={modal==="addRule"?"New Rule":"Edit Rule"} onClose={()=>setModal(null)}
      actions={<><button style={S.btn("ghost")} onClick={()=>setModal(null)}>Cancel</button><button style={S.btn("primary")} onClick={()=>{
