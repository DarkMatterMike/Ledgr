/**
 * DashboardNew.jsx
 * Drop-in replacement Dashboard using the new tiered "Ledger" layout.
 *
 * Receives the same props that App.jsx already has in scope — just
 * pass them in when you swap `const Dashboard = (...)` for:
 *
 *   const Dashboard = (
 *     <DashboardNew
 *       isMobile={isMobile}
 ...
 *       spentByCat={spentByCat} sortedCategories={sortedCategories}
 *       categories={categories} catMap={catMap}
 *       monthTxns={monthTxns} recurringItems={recurringItems}
 *       goals={goals} navigate={navigate}
 *       fmt={fmt} today={today}
 *     />
 *   );
 */
...
export default function DashboardNew({
  ...
  displaySpent,
  spentByCat, sortedCategories,
  categories, catMap,
  monthTxns, recurringItems,
  goals, navigate,
  fmt, today,
}) {
  /* ── Hero toggle: "budget remaining" vs "unbudgeted income" ── */
  const [heroMode, setHeroMode] = useState("budget"); // "budget" | "free"
  const budgetRemaining  = totalBudget - totalSpent;           // remaining within budgets
  const unbudgetedIncome = Math.max(totalIncome - totalBudget, 0); // income not assigned to any category
  ...
  /* ── Recent transactions (last 6 from this month) ────────── */
  ...
  const monoVal = (color = "var(--t1)") => ({
    fontFamily: "var(--font-mono)", fontWeight: 600,
    color, fontSize: isMobile ? 12 : 13,
  });
  const label = {
    fontSize: 10, textTransform: "uppercase",
    letterSpacing: "0.7px", color: "var(--t3)",
    fontFamily: "var(--font-mono)",
  };
  const chip = (bg, border) => ({
    display: "flex", alignItems: "center", gap: 4,
    padding: "3px 10px", borderRadius: 99,
    fontSize: 11, background: bg, border: `1px solid ${border}`,
  });
  const chipN = (color) => ({
  ...
  /* ── Section header ──────────────────────────────────────── */
  function SectionHdr({ ord, title, sub }) {
    return (
      <div style={{ marginBottom: 24, position: "relative" }}>
        ...
        <div style={{
          ...
          paddingBottom: 12, borderBottom: "1px solid rgba(201,149,106,0.12)",
        }}>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, fontWeight: 600, color: "rgba(201,149,106,0.45)", letterSpacing: "1px" }}>
            {ord} ·
          </span>
          <span style={{ fontFamily: "'Playfair Display', serif", fontStyle: "italic", fontWeight: 400, fontSize: isMobile ? 18 : 22, color: "var(--t1)", letterSpacing: "-0.2px" }}>
            {title}
          </span>
          <div style={{ flex: 1, height: 1, background: "linear-gradient(90deg, rgba(201,149,106,0.15), transparent)" }} />
        </div>
        {sub && (
          <div style={{ ...label, marginTop: 5 }}>{sub}</div>
        )}
      </div>
    );
  }
  ...
  function Sig({ status, color }) {
    ...
    return (
      <div style={{
        ...
        boxShadow: status === "over" ? "0 0 0 0 rgba(224,112,112,0.5)" : "none",
        animation: status === "over" ? "sigpulse 2s infinite" : "none",
      }} />
    );
  }

  const totalFreeMoney = heroMode === "budget" ? budgetRemaining : unbudgetedIncome;

  /* ─────────────────────────────────────────────────────────── */
  return (
    <div style={{ fontFamily: "var(--font-body)" }}>

      {/* ── TIER 1: HOW MUCH CAN I SPEND ─────────────────── */}
      <div style={{ ...tier, borderBottom: "1px solid rgba(0,0,0,0.35)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontFamily: "var(--font-disp)", fontWeight: 700, fontSize: isMobile ? 14 : 15, color: "var(--t1)" }}>
              {monthLabel(selectedMonth)}
            </span>
            {isCurrentMonth && (
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, textTransform: "uppercase", letterSpacing: "0.5px", color: "var(--cyan)", background: "rgba(201,149,106,0.1)", padding: "2px 8px", borderRadius: 99 }}>
                current
              </span>
            )}
          </div>
        ...
        {/* Hero number */}
        <div
          onClick={() => setHeroMode(m => m === "budget" ? "free" : "budget")}
          style={{
            fontFamily: "var(--font-mono)", fontWeight: 600,
            fontSize: isMobile ? 52 : 80, letterSpacing: -4, lineHeight: 0.9,
            background: "linear-gradient(135deg, #e8ddd0 12%, #d4a882 55%, #c9956a 100%)",
            WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
            filter: "drop-shadow(0 0 28px rgba(201,149,106,0.2))",
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 14 }}>
          <div style={chip("rgba(224,112,112,0.07)", "rgba(224,112,112,0.2)")}>
            <span style={chipN("var(--red)")}>{catHealth.over}</span>
            <span style={{ color: "var(--t3)" }}>overspent</span>
          </div>
          <div style={chip("rgba(201,149,106,0.07)", "rgba(201,149,106,0.2)")}>
            <span style={chipN("var(--amber)")}>{catHealth.atRisk}</span>
            <span style={{ color: "var(--t3)" }}>at risk</span>
          </div>
          <div style={chip("rgba(109,184,138,0.07)", "rgba(109,184,138,0.2)")}>
            <span style={chipN("var(--green)")}>{catHealth.onTrack}</span>
            <span style={{ color: "var(--t3)" }}>on track</span>
          </div>
        </div>

        {/* Status chips row 2: spent + swap chip */}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
          <div style={chip("rgba(255,255,255,0.04)", "rgba(255,255,255,0.08)")}>
            <span style={chipN("var(--t1)")}>{fmt(totalSpent)}</span>
            <span style={{ color: "var(--t3)" }}>spent</span>
          </div>
          <div
            onClick={() => setHeroMode(m => m === "budget" ? "free" : "budget")}
            style={{
              ...chip(
                heroMode === "budget" ? "rgba(109,184,138,0.07)" : "rgba(201,149,106,0.07)",
                heroMode === "budget" ? "rgba(109,184,138,0.2)"  : "rgba(201,149,106,0.2)"
              ),
              cursor: "pointer",
            }}>
            <span style={chipN(heroMode === "budget" ? "var(--green)" : "var(--amber)")}>
              {heroMode === "budget" ? `${fmt(unbudgetedIncome)} free` : `${fmt(budgetRemaining)} left`}
            </span>
            <span style={{ color: "var(--t3)" }}>
              {heroMode === "budget" ? "unbudgeted" : "in budget"}
            </span>
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 600, color: "var(--cyan)" }}>
              {fmt(dailyBurn)} / day
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3, textAlign: "right" }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 600, color: "var(--cyan)" }}>
              {daysUntilOut != null ? `~ ${daysUntilOut} days` : "—"}
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={tier}>
        ...
        {categories.length === 0 ? (
          <div className="ledgr-empty">
            <div className="ledgr-empty-icon">🏷️</div>
            <div className="ledgr-empty-title">No categories yet</div>
            <div>Add a category to start tracking budgets</div>
          </div>
        ) : (
          <div>
      ...
      <div style={tier}>
        ...
        {monthTxns.length === 0 ? (
          <div className="ledgr-empty">
            ...
            <div className="ledgr-empty-title">No transactions yet</div>
            <div>Sync your accounts or add transactions manually</div>
          </div>
        ) : isMobile ? (
          ...
          <div>
            {recentTxns.map(txn => (
              <div key={txn.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid rgba(255,255,255,0.032)" }}>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 600, color: txn.amount < 0 ? "var(--red)" : "var(--green)", flexShrink: 0 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
            {[recentTxns.slice(0, 3), recentTxns.slice(3, 6)].map((group, gi) => (
              <div key={gi}>
                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", fontSize: 12, color: "var(--t3)", marginBottom: 10, paddingBottom: 7, borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                  {gi === 0 ? "This week" : "Earlier this month"}
                </div>
                {group.map(txn => (
                  <div key={txn.id} style={{ display: "flex", alignItems: "center", gap: 11, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.032)" }}>
                    ...
                    <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 600, color: txn.amount < 0 ? "var(--red)" : "var(--green)", flexShrink: 0 }}>
                      {txn.amount < 0 ? "−" : "+"}{fmt(Math.abs(txn.amount))}
                    </div>
                  </div>
                ))}
        ...
        <button onClick={() => navigate("transactions")} style={{ marginTop: 14, background: "none", border: "none", color: "var(--t3)", fontSize: 11, cursor: "pointer", fontFamily: "var(--font-mono)" }}>
          All transactions →
        </button>
      </div>
            ...
            {upcomingItems.map(item => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 0", borderBottom: "1px solid rgba(255,255,255,0.032)" }}>
                <div style={{
                  ...
                  background: item.type === "income" ? "rgba(109,184,138,0.07)" : "rgba(255,255,255,0.04)",
                  border: `1px solid ${item.type === "income" ? "rgba(109,184,138,0.2)" : "rgba(255,255,255,0.07)"}`,
                  display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                }}>
                  <div style={{ fontSize: 7, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.4px", color: item.type === "income" ? "var(--green)" : "var(--t3)", background: item.type === "income" ? "rgba(109,184,138,0.12)" : "rgba(255,255,255,0.04)", width: "100%", textAlign: "center", padding: "1px 0" }}>MAY</div>
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 14, fontWeight: 600, color: item.type === "income" ? "var(--green)" : "var(--t2)", lineHeight: 1.2 }}>{item.recurringDay}</div>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 600, color: item.type === "income" ? "var(--green)" : "var(--red)", flexShrink: 0 }}>
                  {item.type === "income" ? "+" : "−"}{item.amountMin != null ? fmt(item.amountMin) : "—"}
                </div>
              </div>
            ))}
          </div>
        ) : (
          /* Desktop: two-column bills / incoming */
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }}>
            <div>
              ...
              {upcomingItems.filter(i => i.type !== "income").map(item => (
                <div key={item.id} style={{ display: "flex", alignItems: "center", gap: 13, padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.032)" }}>
                  ...
                  <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 600, color: "var(--red)", flexShrink: 0 }}>
                    ...
                    {item.amountMin != null && item.amountMax != null && item.amountMax !== item.amountMin ? `–${fmt(item.amountMax)}` : ""}
                  </div>
                </div>
              ))}
              {upcomingItems.filter(i => i.type !== "income").length === 0 && (
                <div style={{ fontSize: 12, color: "var(--t3)" }}>No bills due</div>
              )}
            </div>
            <div>
              ...
              {upcomingItems.filter(i => i.type === "income").length === 0 && (
                <div style={{ fontSize: 12, color: "var(--t3)" }}>No income due this month</div>
              )}
            </div>
          </div>
        )}
